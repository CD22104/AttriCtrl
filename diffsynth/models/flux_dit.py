...
class FluxJointAttention(torch.nn.Module):
    ...
    def apply_rope(self, xq, xk, freqs_cis):
        # 在 flux_dit.py 的 apply_rope 函数开头加：
...
class FluxDiT(torch.nn.Module):
    ...
    def process_entity_masks(self, hidden_states, prompt_emb, entity_prompt_emb, entity_masks, text_ids, image_ids, attri_value_emb=None, attri_len=None):
        ...
        if entity_masks is not None:
            ...
            for i in range(max_masks):
                entity_emb = entity_prompt_emb[:, i, None].squeeze(1)  # Shape: [B, L_entity, D]
                if attri_value_emb is not None:
                    entity_emb = torch.cat([entity_emb, attri_value_emb.unsqueeze(0)], dim=1)
                local_embs.append(entity_emb)
    ...
    def forward(
        ...
        use_gradient_checkpointing=False, attri_value_emb=None, attri_len=None,
        **kwargs
    ):
        if tiled:
