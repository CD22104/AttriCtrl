    ...
    def __call__(
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            inference_callback = lambda prompt_emb_posi, controlnet_kwargs: lets_dance_flux(
                ...
                hidden_states=latents, timestep=timestep, attri_value_emb = attri_value_emb, attri_len=attri_len, 
...
def lets_dance_flux(
    ...
    **kwargs
    ...
    # add AttriCtrl
    if attri_value_emb is not None:
        attri_len = attri_value_emb.size(0)
        padding_text = torch.zeros(1, attri_len, 3).to(device=text_ids.device, dtype=text_ids.dtype)
        text_ids = torch.cat([text_ids, padding_text], dim=1)
        prompt_emb = torch.cat([prompt_emb, attri_value_emb.unsqueeze(0)], dim=1)
