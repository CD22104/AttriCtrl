...
from diffsynth.attrictrl.attri_encoder import AttriEncoder
...
class FluxImagePipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.float16):
        ...
        # add AttriCtrl
        self.attri_encoder: AttriEncoder = None
        self.model_names = ['text_encoder_1', 'text_encoder_2', 'dit', 'vae_decoder', 'vae_encoder', 'controlnet', 'ipadapter', 'ipadapter_image_encoder', 'attri_encoder']
    ...
    def __call__(
        ...
        # add AttriCtrl
        # if attri_value is not None:
        #     attri_value_tensor = torch.tensor([attri_value], dtype=self.torch_dtype, device=self.device)
        #     attri_value_emb = self.attri_encoder(attri_value_tensor*1000, self.torch_dtype)
        # else:
        #     attri_value_emb = None
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            inference_callback = lambda prompt_emb_posi, controlnet_kwargs: lets_dance_flux(
                ...
                hidden_states=latents, timestep=timestep, attri_value_emb = attri_value, attri_len=attri_len, attri_type=attri_type,
...
def lets_dance_flux(
    ...
    **kwargs
    ...
    # add AttriCtrl
    padding_text = torch.zeros(1, attri_len*len(attri_value_emb), 3).to(device=text_ids.device, dtype=text_ids.dtype)
    text_ids = torch.cat([text_ids, padding_text], dim=1)
    for i in range(len(attri_value_emb)):
        value_emb = attri_value_emb[i]
        prompt_emb = torch.cat([prompt_emb, value_emb.unsqueeze(0)], dim=1)

    if entity_prompt_emb is not None and entity_masks is not None:
        prompt_emb, image_rotary_emb, attention_mask = dit.process_entity_masks(hidden_states, prompt_emb, entity_prompt_emb, entity_masks, text_ids, image_ids, attri_value_emb, attri_len)
    else:
