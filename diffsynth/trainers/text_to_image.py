...
class LightningModelForT2ILoRA(pl.LightningModule):
    ...
    def freeze_parameters(self,other_model):
        # Freeze parameters
        self.target_module = other_model
        self.pipe.requires_grad_(False)
        self.pipe.eval()
        self.pipe.denoising_model().train()
        self.target_module.train()
    ...
    def training_step(self, batch, batch_idx):
        # add AttriCtrl
        text, image, attri_value = batch["text"], batch["image"], batch["attri_value"]
        ...
        if attri_value is not None:
            attri_value_emb = self.pipe.attri_encoder(attri_value*1000, noisy_latents.dtype)

        # Compute loss
        noise_pred = self.pipe.denoising_model()(
            noisy_latents, timestep=timestep, **prompt_emb, **extra_input,
            use_gradient_checkpointing=self.use_gradient_checkpointing,
            attri_value_emb=attri_value_emb, attri_len=self.pipe.attri_encoder.attri_len,
        )
    ...
    def configure_optimizers(self):
        # trainable_modules = filter(lambda p: p.requires_grad, self.target_module.parameters())
        # trainable_modules = filter(lambda p: p.requires_grad, self.pipe.denoising_model().parameters())
        trainable_modules = filter(
            lambda p: p.requires_grad,
            chain(
                self.pipe.denoising_model().parameters(),
                self.target_module.parameters()
            )
        )
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        trainable_param_dit = list(filter(lambda named_param: named_param[1].requires_grad, self.pipe.denoising_model().named_parameters()))
        trainable_param_dit = set([named_param[0] for named_param in trainable_param_dit])
        state_dict_dit = self.pipe.denoising_model().state_dict()
        lora_state_dict = {}
        for name, param in state_dict_dit.items():
            if name in trainable_param_dit:
        ...
        trainable_param_names = list(filter(lambda named_param: named_param[1].requires_grad, self.target_module.named_parameters()))
        trainable_param_names = set([named_param[0] for named_param in trainable_param_names])
        state_dict = self.target_module.state_dict()
        for name, param in state_dict.items():
            if name in trainable_param_names:
                new_name = f"attri_encoder.{name}"
                lora_state_dict[new_name] = param
